...
def attack_riders(this_vars):
    ...
    this_vars.B = this_vars.B-(response_time*40)-80
...
def circle_wagons(this_vars):
    response_time = or78_helpers.shooting(this_vars.D9)
    this_vars.B = this_vars.B-(response_time*30)-80
    this_vars.M -= 25
    shooting_outcome(response_time, this_vars)
...
def riders(this_vars):
    if random.random()*10 > ((this_vars.M/100-4)**2+72)/((this_vars.M/100-4)**2+12)-1:
        return
    this_vars.S5 = random.random() < .8
    S5_message = "" if this_vars.S5 else "DON'T "
    message = "RIDERS AHEAD.   THEY {}LOOK HOSTILE".format(S5_message)
    print(message)
    if random.random() <= .2:
        this_vars.S5 = not this_vars.S5
    choice = 0
